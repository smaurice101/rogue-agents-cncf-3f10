Scaling [rogue-agents-cncf-3f10] With Kubernetes
===========================

Generated On: 2026-09-15 17:30:52 UTC
...
Based on your TML solution [rogue-agents-cncf-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`rogue-agents-cncf-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-rogue-agents-cncf-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingressrogue-agents-cncf-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f rogue-agents-cncf-3f10.yml

rogue-agents-cncf-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: rogue-agents-cncf-3f10.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/smaurice101/raspberrypitss/tree/main/tml-airflow/dags/tml-solutions/rogue-agents-cncf-3f10/ymls/rogue-agents-cncf-3f10
...
.. code-block:: YAML
   ...
   ################# rogue-agents-cncf-3f10.yml
     ...
     kind: Deployment
     metadata:
       name: rogue-agents-cncf-3f10
     spec:
       selector:
         matchLabels:
           app: rogue-agents-cncf-3f10
       ...
       template:
         ...
         spec:
           containers:
           - name: rogue-agents-cncf-3f10
             image: maadsdocker/rogue-agents-cncf-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'rogue-agents-cncf-3f10'                              
             - name: SOLUTIONNAME
               value: 'rogue-agents-cncf-3f10'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag-rogue-agents-cncf-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
             ...
             - name: step3localfileinputfile # STEP 3 localfile inputfile field can be adjusted here.
               value: '/rawdatademo/rogueagentsdata.txt'
             ...
             - name: step1solutiontitle # STEP 1 solutiontitle field can be adjusted here. 
               value: 'Rogue Agent Monitoring Solution'                              
   ...
   ---
     ...
     kind: Service
     metadata:
       name: rogue-agents-cncf-3f10-visualization-service
       labels:
         app: rogue-agents-cncf-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/rogue-agents-cncf-3f10 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/rogueagents2.html?topic=iot-preprocess&offset=-1&groupid=&rollbackoffset=50&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-rogue-agents-cncf-3f10.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-rogue-agents-cncf-3f10.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-rogue-agents-cncf-3f10.yml
...
.. code-block::
   ...
   http://tml.tss/viz/rogueagents2.html?topic=iot-preprocess&offset=-1&groupid=&rollbackoffset=50&topictype=prediction&append=0&secure=1
